...
;			ONLY_HV   If set, the data will be troncated to have only the data corresponding to a HV = 200V
;
; Example:
...
;
; History:	
;     2018-Sep-26 Sophie Changed keyword year into only_hv for more accurate description of the keyword, that will be set for FOXSI2 and later flights
;                        Added CdTe keyword to the call to get_payloard_coords
...
FUNCTION	FOXSI_LEVEL0_TO_LEVEL1, FILENAME, DETECTOR = DETECTOR, STOP = STOP, $
			GROUND = GROUND, ONLY_HV = ONLY_HV, CDTE = CDTE

	default, ONLY_HV, 1
;	if not keyword_set(filename) then filename = 'data_2012/foxsi_level0_data.sav' ; Is this really useful since filename is an input?
...
;	data_struct.assoc_adc[0,*,0] = assoc_adc[*,0,*]
	...
	if ONLY_HV eq 1 then data_struct = data_struct[ where( data_struct.hv eq 200 ) ]
